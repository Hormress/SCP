...
在传统现实扭曲理论中，现实强度（reality strength）这一概念先于现实学这一学科诞生。相对现实强度最早被定义为一个无量纲常数：
...
1977年，SCP基金会的C. Rzewski教授在三波特兰各现实门径分别进行了许多基本物理常数的测量，在此期间他敏锐地观察到：测得的精细结构常数 $alpha$ 在隔离现实扭曲效应的空间中严格恒定，但不同空间内存在极小的偏差；空间内存在现实扭曲时，精细结构常数 $alpha$ 不恒定。这一发现就是我们今天熟知的Rzewski定律。

根据Rzewski定律，J. Caldmann和C. Rzewski在1983年引入了我们今天依然通用的绝对现实强度 $cal(H)$。在进行了相当精密的测量后，$cal(H)$ 由局域精细结构常数 $alpha_s$ 与基准精细结构常数 $alpha_0$ 如下定义：
...
这几条假设构成了结构现实学提倡的基本思想。T. Y. Minovsky和Ionesco在2019年正式发表了结构现实学基本模型的原始形式，这一模型将现实描述为一个9维扭量场，因此一般称为Sigma-9模型。

== 现实学背景的奇术理论

Ionesco-Ginzburg效应会在现实结构中产生一种准粒子冲子（élanon），经典奇术模型中假设的生命力能量（élan-vital energy）本质上是冲子的传递。
...
2014年7月19日午后，澳大利亚悉尼市民Samuel Wilkinson在坎贝尔敦遭遇一起抢劫。
